...
//When water level drops below specific level (PUMP_TRIGGER) pump turns on
//Pump cannot run for longer than specified in PUMP_CUTOFF. When pump cuts off, it can no longer be triggered
void refill_water()
{
    if (check_water_level() > PUMP_TRIGGER && !pump_error){
        if(!is_pump_running_since_set){
            pump_running_since = millis();
            is_pump_running_since_set = true;
        } 
        else if(millis() - pump_running_since < PUMP_CUTOFF) digitalWrite(RELAY_PUMP_PIN, LOW); //safety check, pump cannot run for longer than specified in PUMP_CUTOFF
        else{
...
void display_water_level()
{
    ...
    if(water_lvl >= NO_WATER) lcd.print("    NO WATER    ");
    else if(water_lvl >= LITTLE_OF_WATER) lcd.print("LITTLE OF WATER ");
    else if(water_lvl >= OPTIMUM_OF_WATER) lcd.print("OPTIMUM OF WATER");
    else lcd.print(" LOTS OF WATER  ");
}
...
void display_combined_view()
{
    ...
    int water_lvl = check_water_level();
    lcd.print(char(1));
    if(water_lvl >= NO_WATER) lcd.print("NONE  ");
    else if(water_lvl >= LITTLE_OF_WATER) lcd.print("LITTLE");
    else if(water_lvl >= OPTIMUM_OF_WATER) lcd.print("OPTIM ");
    else lcd.print("LOTS  ");
